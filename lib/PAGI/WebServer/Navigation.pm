...
sub _render_tree_level {
    ...
    for my $key (@sorted_keys) {
        ...
        $html .= qq|    <a href="$node->{path}" class="$link_class spectrum-Link spectrum-Link--primary spectrum-Link--quiet">\n|;
        $html .= qq|      <span class="spectrum-TreeView-itemLabel">$node->{title}</span>\n|;
        $html .= qq|    </a>\n|;
        ...
        if ($has_children) {
            ...
            $html .= $self->_render_tree_level($node->{children}, $current_path, $node_path, $depth + 1);
            $html .= qq|    </ul>\n|;
        }
