...
has external_min_interval => ( is => 'ro', default => 1.0 );  # seconds between requests per host
has external_jitter       => ( is => 'ro', default => 0.2 );  # random 0..jitter seconds
has external_host_slots   => ( is => 'ro', default => 1 );    # max concurrent requests per host (usually 1)
  ...
  while (1) {
    ...
    eval {
      ...
      $res = {
        ...
        anchor_refs    => [],
        broken         => {
          $u => { status => 0, error => "worker exception: $err", occurrences => 1 }
        },
      };
    };
