...
sub run_job ($self, $job) {
  ...
  eval {
    $self->logger->info(sprintf(
      'FETCH start mode "%s" for url "%s" on PID %s; ',
      $job->{mode}, $url, $$,
    ));
  ...
  # Dedupe
  my %di;
  @found_internal = grep { !$di{$_}++ } @found_internal;
  my %de;
  @found_external = grep { !$de{$_}++ } @found_external;
...
sub extract_links ($dom, $page_uri) {
  ...
  for my $e (
    $dom->find(
'svg use[href], svg use[xlink\:href], svg image[href], svg image[xlink\:href]'
    )->each
  ) {
    push @out, $e->{href}         if defined $e->{href};
    push @out, $e->{'xlink:href'} if defined $e->{'xlink:href'};
  ...
  for my $e (
    $dom->find(
'svg image[href], svg image[xlink\:href], svg script[href], svg script[xlink\:href]'
    )->each
  ) {
    push @out, $e->{href}         if defined $e->{href};
    push @out, $e->{'xlink:href'} if defined $e->{'xlink:href'};
  ...
  # Resources
  for
    my $e ($dom->find('img[src], script[src], iframe[src], source[src]')->each)
  {
...
sub _fetch_with_retries ($loop, $http, $url, $job) {
  ...
  for (my $try = 1; $try <= $job->{max_retries}; $try++) {
    ...
    my $req_f = $http->GET(
      ...
      headers => [
        'accept' =>
          'text/html,application/xhtml+xml;q=0.9,image/svg+xml;q=0.9,*/*;q=0.8',
      ],
    );
